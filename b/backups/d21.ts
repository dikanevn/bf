...
function updateCoefficients() {
    try {
        ...
        const initialValueHex = '0x58d15e17628000'; // Начальное значение для 1 раунда (25 * 10^15)
        const decayFactorHex = '0x38b389129d800';   // Коэффициент уменьшения (0.9975 * 10^15)
        ...
        const decayNumerator = BigInt(decayFactorHex);
        const decayDenominator = BigInt(decayDenominatorHex);

        // Путь к файлу
        const filePath = './d02.json';
        ...
        console.log(`Коэффициент уменьшения (hex): ${decayFactorHex}`);
        console.log(`Знаменатель (hex): ${decayDenominatorHex}`);
        ...
        const updatedData = data.map(item => {
            ...
            // Математически это эквивалентно умножению на 0.9975:
            // (value * 0x38b389129d800) / 0x38d7ea4c68000 = value * (0.9975 * 10^15) / (10^15) = value * 0.9975
            currentValueBigInt = (currentValueBigInt * decayNumerator) / decayDenominator;
