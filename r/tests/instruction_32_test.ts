/**
 * Тест для инструкции 32: Создание и минтинг pNFT с проверкой Merkle-доказательства
 * 
 * Этот тест проверяет функциональность создания и минтинга Programmable NFT (pNFT)
 * с верификацией через Merkle-дерево. Используются данные из раунда для построения
 * Merkle-дерева и проверки, что адрес пользователя находится в списке разрешенных
 * для минтинга. Создаются все необходимые аккаунты для pNFT в одной транзакции.
 */
import { 
  Connection, 
  Keypair, 
...
describe('Instruction 32', function() {
  ...
  it('should create and mint a pNFT with Merkle proof verification', async function() {
    console.log('Начинаем тест создания и минта pNFT с проверкой Merkle proof (инструкция 32)');
