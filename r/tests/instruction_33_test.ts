/**
 * Тест для инструкции 33: Создание и минтинг pNFT с проверкой Merkle-доказательства и метаданными
 * 
 * Этот тест проверяет функциональность создания и минтинга Programmable NFT (pNFT)
 * с верификацией через Merkle-дерево и расширенными метаданными. Используются данные
 * из раунда для построения Merkle-дерева и проверки, что адрес пользователя находится
 * в списке разрешенных для минтинга. Создаются все необходимые аккаунты для pNFT
 * с дополнительными метаданными в одной транзакции.
 */
import { 
  Connection, 
  Keypair, 
